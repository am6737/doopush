...
// AppKeyAuth 仅用于客户端首次注册。App Key 是公开接入标识，不授予服务端权限。
...
// InstallationAuth 将客户端操作限制到注册时签发的安装实例凭证。
func InstallationAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		appID, ok := pathAppID(c)
		if !ok {
			return
		...
		if len(parts) != 2 || parts[0] != "Bearer" || !strings.HasPrefix(parts[1], models.InstallationTokenPrefix) {
			response.Unauthorized(c, "缺少Installation Token")
			c.Abort()
			return
		}
		var credential models.InstallationCredential
		err := database.DB.Where("app_id = ? AND token_hash = ? AND status = 1", appID, utils.HashCredential(parts[1])).First(&credential).Error
		if err != nil || (credential.ExpiresAt != nil && credential.ExpiresAt.Before(time.Now())) {
			response.Unauthorized(c, "Installation Token无效或已过期")
			c.Abort()
			return
		}
		var app models.App
		...
		c.Set("app_id", appID)
		c.Set("device_id", credential.DeviceID)
		c.Set("installation_credential_id", credential.ID)
		c.Set("auth_type", "installation")
		now := time.Now()
		_ = database.DB.Model(&credential).Where("last_used_at IS NULL OR last_used_at < ?", now.Add(-5*time.Minute)).Update("last_used_at", now).Error
		c.Next()
	}
}
