...
// SendBroadcast 广播推送
// @Summary 广播推送
// @Description 向应用的所有设备发送推送通知。支持JWT Token和具备push:send、push:broadcast的App Secret
// @Tags 推送管理
// @Accept json
// @Produce json
// @Security BearerAuth
...
// @Failure 400 {object} response.APIResponse "请求参数错误"
// @Failure 401 {object} response.APIResponse "Installation Token无效"
...
func (ctrl *PushController) ReportPushStatistics(ctx *gin.Context) {
	...
	var installationDevice models.Device
	if err := database.DB.Select("id", "token_hash").Where("id = ? AND app_id = ? AND status = 1", ctx.GetUint("device_id"), appID).First(&installationDevice).Error; err != nil || installationDevice.TokenHash != utils.HashString(req.DeviceToken) {
		response.Forbidden(ctx, "只能上报当前安装实例的推送统计")
		return
	}
