...
func (a *AppController) GetAppAPIKeys(c *gin.Context) {
	...
	apiKeys, err := a.appService.GetAppAPIKeys(uint(appID), c.GetUint("user_id"))
	if err != nil {
		if err.Error() == "无权限访问该应用" {
			response.Forbidden(c, err.Error())
		} else {
...
func (a *AppController) CreateAPIKey(c *gin.Context) {
	...
	_ = a.auditService.LogActionWithBeforeAfter(userID, &appIDUint, "create", "api_key", strconv.FormatUint(uint64(keyInfo.ID), 10), nil, gin.H{
		"name": req.Name, "key_prefix": keyInfo.KeyPrefix, "key_suffix": keyInfo.KeySuffix,
	}, c.ClientIP(), c.GetHeader("User-Agent"))
	c.JSON(http.StatusCreated, response.APIResponse{Code: 201, Message: "API密钥创建成功", Data: CreateAPIKeyResponse{
		APIKey: apiKey, KeyInfo: keyInfo, Warning: "请妥善保存API密钥，再次获取将无法查看完整密钥",
	}})
}
...
func (a *AppController) DeleteAPIKey(c *gin.Context) {
	...
	appIDUint := uint(appID)
	_ = a.auditService.LogActionWithBeforeAfter(userID, &appIDUint, "delete", "api_key", strconv.FormatUint(keyID, 10), gin.H{
