...
// RegisterDevice 注册设备
// @Summary 注册设备
// @Description 注册设备以接收推送通知。需要验证App Key属于指定应用且bundle_id与应用包名匹配。可以在注册时同时设置设备标签
// @Tags 设备管理
// @Accept json
// @Produce json
// @Security AppKeyAuth
...
func (d *DeviceController) RegisterDevice(c *gin.Context) {
	...
	installationID, installationToken, err := d.credentialService.IssueInstallationCredential(uint(appID), device.ID)
	if err != nil {
...
func (d *DeviceController) GetDevice(c *gin.Context) {
	...
	if deviceID, err := strconv.ParseUint(deviceParam, 10, 32); err == nil {
		...
		if err := services.EnrichOnlineStatusOne(c.Request.Context(), d.rdb, device); err != nil {
			log.Printf("EnrichOnlineStatusOne failed: %v", err)
		}
