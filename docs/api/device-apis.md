...
| 接口 | 描述 | 权限要求 |
|------|------|----------|
| [注册设备](#注册设备) | 注册设备以接收推送通知 | App Key |
| [查询设备列表](#查询设备列表) | 获取应用的设备列表 | JWT |
| [查询设备详情](#查询设备详情) | 获取指定设备的详细信息 | JWT |
| [更新设备状态](#更新设备状态) | 启用或禁用设备推送 | JWT |
| [删除设备](#删除设备) | 删除设备记录 | JWT |
...
- **设备注册** (`POST /apps/{appId}/devices`)：使用 **App Key** (`X-App-Key`)，由 SDK / 服务端在握手前调用。
- **管理类查询 / 操作**（设备列表、设备详情、状态变更、删除、标签）：使用登录后获得的 **JWT Token** (`Authorization: Bearer`)。

App Key 不再分级，无 `device` / `push` 等权限粒度。
...
注册设备以接收推送通知。需要验证 App Key 属于指定应用且 bundle_id 与应用包名匹配。
...
| 401 | 401 | App Key无效或与应用不匹配 | 检查App Key和应用ID |
