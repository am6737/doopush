...
| | [上报推送统计](#上报推送统计) | 客户端上报点击 / 送达事件 | Installation Token |
...
- **客户端上报接口**（`POST /push/statistics/report`）：使用注册时签发的 **Installation Token**（`Authorization: Bearer`）

Installation Token 只能代表一个安装实例，不能读取统计或操作其他设备。
...
curl -X POST "https://doopush.com/api/v1/apps/123/push/statistics/report" \
     -H "Authorization: Bearer dp_ins_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" \
...
| 401 | 401 | 未认证 | 检查 JWT 或 Installation Token 是否有效 |
