...
- **多平台支持** - iOS APNs 与 Android 多厂商推送（FCM / 华为 / 荣耀 / 小米 / OPPO / vivo / 魅族）
...
2. 填写注册信息：
   - **用户名** - 用于登录的唯一标识
   - **邮箱地址** - 用户唯一标识，必填且需符合邮箱格式
   - **密码** - 至少6位字符，建议包含字母和数字
   - **确认密码** - 重复输入密码确认
3. 点击 **"注册"** 按钮
4. 注册成功后系统会自动登录并跳转到主页面
5. 仅当自动登录失败时，才会跳转登录页，需用刚注册的用户名和密码再次登录

::: tip 💡 提示
当前注册无邮箱验证流程，提交后即可登录。请妥善保存自己的用户名与密码——平台暂未上线邮箱找回密码，遗忘后只能在数据库直接重置。
:::
...
2. 填写应用信息：
   ...
   - **应用描述** - 简单描述应用功能（可选）
   - **应用图标** - 上传应用图标（可选，支持 JPEG/PNG/GIF/WebP/SVG，最大 5MB）
...
创建应用时系统会自动生成 App Key。打开应用的配置页面即可查看并复制，它用于客户端 SDK 注册设备、连接 Gateway 和上报统计。

::: warning 🔒 安全提醒
App Key 可以包含在客户端中，但不能用于发送推送或访问服务端数据。服务端调用请创建带最小权限的 App Secret。
:::
...
- [**消息模板**](/guide/push.md#使用消息模板) - 创建可复用的推送模板
