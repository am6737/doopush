...
- Successful device registration rotates and returns a per-device Installation Token. Native SDKs persist and use it transparently.

## Trust boundaries

```text
App ID + App Key       -> register one installation
Installation Token    -> operate as that installation
...
Installation Token  dp_ins_<random>
```
