...
import type { AppAPIKey, AppSecret } from '@/types/api'
...
export function AppConfigTab() {
  ...
  const loadApiKeys = useCallback(async () => {
    ...
    try {
      ...
      setApiKeys(await AppService.getAppAPIKeys(currentApp.id))
    } catch (error) {
      toast.error((error as Error).message || '加载 API 密钥失败')
  ...
  return <div className="space-y-6">
    ...
    {(currentApp.role === 'owner' || currentApp.role === 'developer') && <Card>
      <CardHeader className="flex-row items-start justify-between gap-4">
        <div><CardTitle className="flex items-center gap-2"><KeyRound className="h-5 w-5" />API 密钥</CardTitle><CardDescription className="mt-1">保留原有创建、查看和删除能力；当前不参与 SDK 注册、WebSocket 或服务端 API 鉴权。</CardDescription></div>
        <Button variant="outline" onClick={() => setCreateApiKeyOpen(true)}><Plus className="mr-2 h-4 w-4" />创建 API 密钥</Button>
      </CardHeader>
      <CardContent>
        {apiKeysLoading ? <div className="py-8 text-center text-muted-foreground">加载中...</div> : apiKeys.length === 0 ? <div className="py-8 text-center text-muted-foreground">暂无 API 密钥</div> : <Table>
          <TableHeader><TableRow><TableHead>名称</TableHead><TableHead>密钥</TableHead><TableHead>创建时间</TableHead><TableHead className="text-right">操作</TableHead></TableRow></TableHeader>
          <TableBody>{apiKeys.map((apiKey) => <TableRow key={apiKey.id}><TableCell className="font-medium">{apiKey.name}</TableCell><TableCell><code className="text-sm text-muted-foreground">{apiKey.key_prefix}...{apiKey.key_suffix}</code></TableCell><TableCell className="text-sm text-muted-foreground">{new Date(apiKey.created_at).toLocaleString('zh-CN')}</TableCell><TableCell className="text-right"><Button size="icon" variant="ghost" title="删除 API 密钥" onClick={() => { setSelectedApiKey(apiKey); setDeleteApiKeyOpen(true) }}><Trash2 className="h-4 w-4" /></Button></TableCell></TableRow>)}</TableBody>
        </Table>}
      </CardContent>
    </Card>}
    ...
    <CreateApiKeyDialog app={currentApp} open={createApiKeyOpen} onOpenChange={setCreateApiKeyOpen} onSuccess={loadApiKeys} />
    <DeleteApiKeyDialog apiKey={selectedApiKey} open={deleteApiKeyOpen} onOpenChange={setDeleteApiKeyOpen} onSuccess={loadApiKeys} />
  </div>
}
